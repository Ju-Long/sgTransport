...
module.exports = async (fastify, opts) => {
    ...
    fastify.get('/bus/distance/Bus/:ServiceNo/:StartBusStopCode/:EndBusStopCode', async (request, reply) => {
        try {
            ...
            if (!StartBusStopCode || !EndBusStopCode || !ServiceNo) {
                throw new RequestError('invalid Bus Stop Codes input', {StartBusStopCode: StartBusStopCode, EndBusStopCode: EndBusStopCode}, new Error().stack);
            }
            ...
            if (bus_in_start_bus_stop === 0) {
                throw new RequestError('no Start Bus Stop Code can be found', {StartBusStopCode: StartBusStopCode}, new Error().stack);
            ...
            if (bus_in_end_bus_stop === 0) {
                throw new RequestError('no End Bus Stop Code can be found', {EndBusStopCode: EndBusStopCode}, new Error().stack);
            }
            ...
            if (start_bus_route_index < 0) {
                throw new RequestError('Start Bus Stop Code Not found on route', {BusStopCode: StartBusStopCode, bus: bus}, new Error().stack);
            }
    
            let end_bus_route_index = bus.Route.findIndex((bus_stop) => { return bus_stop.BusStopCode === EndBusStopCode });
            if (end_bus_route_index < 0) {
                throw new RequestError('End Bus Stop Code Not found on route', {ServiceNo: bus.ServiceNo, BusStopCode: StartBusStopCode}, new Error().stack);
            }
    ...
    fastify.get('/bus/distance/BusStop/:StartBusStopCode/:EndBusStopCode', async (request, reply) => {
        try {
            ...
            for (let i in buses_available) {
                ...
                let start_bus_route_index = bus.Route.findIndex((bus_stop) => { return bus_stop.BusStopCode === StartBusStopCode });
                if (start_bus_route_index < 0) {
                    throw new RequestError('Start Bus Stop Code Not found on route', {StartBusStopCode: StartBusStopCode, bus: bus}, new Error().stack);
                }
                let end_bus_route_index = bus.Route.findIndex((bus_stop) => { return bus_stop.BusStopCode === EndBusStopCode });
                if (end_bus_route_index < 0) {
                    throw new RequestError('End Bus Stop Code Not found on route', {bus: bus, BusStopCode: StartBusStopCode}, new Error().stack);
                }
    ...
    fastify.get('/bus/timing/BusStop/:BusStopCode', async (request, reply) => {
        try {
            ...
            if (!BusStopCode) {
                throw new RequestError('invalid Bus Stop Code input', {BusStopCode: BusStopCode}, new Error().stack);
            }
    ...
    fastify.get('/bus/search/Bus/:ServiceNo', async (request, reply) => {
        try {
            const ServiceNo = request.params.ServiceNo
            if (!ServiceNo) {
                throw new RequestError('invalid Service No input', {ServiceNo: ServiceNo}, new Error().stack);
            }
            ...
            if (buses.length < 1) {
                throw new RequestError('Service Number Could not be found', {ServiceNo: ServiceNo}, new Error().stack);
            }
    ...
    fastify.get('/bus/search/BusStop/:BusStopCode', async (request, reply) => {
        try {
            ...
            if (bus_stops.length < 1) {
                throw new RequestError('Bus Stop Code Could not be found', {BusStopCode: BusStopCode}, new Error().stack);
            }
    ...
    fastify.get('/bus/nearest/:page', async (request, reply) => {
        try {
            ...
            if (isNaN(lat) || isNaN(long)) {
                throw new RequestError('invalid coordinates input', {lat: lat, long: long}, new Error().stack);
            }
    
            if (isNaN(page)) {
                throw new RequestError('invalid page input', {page: page}, new Error().stack);
            }
    
            if (Number(page) < 1) {
                throw new RequestError('invalid page number, min page is 1', {page: page}, new Error().stack);
            }
